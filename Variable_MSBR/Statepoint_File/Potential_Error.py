...
def Mean_Lifetime_Plot(SYSTEM="",n=200,k_range=[0.7,1.3]):
    ...
    for j in [0,1]:
        plt.scatter(k_Data[j,:],Mean_Lifetime[j,:],label=f"{FTB[j]}&D")
        plt.scatter(k_Data[j,:],Mean_Lifetime[j+2,:],label=f"{FTB[j]}&P")
    plt.title(f"Mean Lifetime for {SYSTEM} vs k-static (delayed)")
    plt.yscale("log")
    plt.legend()
    plt.grid()
