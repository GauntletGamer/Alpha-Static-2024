...
def System_Data_Plot(SYSTEM="",FTB="Both",n=200,TEXT_FILE=False,k_range=[0.7,1.3]):
    ...
    if SYSTEM == "J":
        ...
        sys_u = "%"
    elif SYSTEM == "CUSTOM":        # Replace CUSTOM with 1-2 letter system identifier, also add to System ID input above
        Crit_Vals = []              # Fill tuple with critical values as follows: [Fast_val,Thermal_val]
        Var_Multi = np.array([])    # Fill array with multiplier values, see above for examples
        sys_u = ""                  # Rename string to unit of variation (ex:cm,%)
    ...
    if any(_=="Fa" for _ in FTB):
        ...
        plt.grid(zorder=0)
        plt.legend()
        plt.savefig("Alpha_vs_K:Fast.png",dpi=1500)
        plt.close()
    ...
    if TEXT_FILE:
        ...
        for i in range(len(Var_Multi)):
            txt_file.write(f"{k_Data[0,i]} {Reactivity[0,i]} {kp_Data[0,i]}\n")
        txt_file.write("\n")
        for i in range(len(Var_Multi)):
            txt_file.write(f"{k_Data[1,i]} {Reactivity[1,i]} {kp_Data[0,i]}\n")
        txt_file.close()

System_Data_Plot(TEXT_FILE=True,k_range=[0.9,1.1])
