...
def SubPlot_Spectrum(ax,sp=openmc.StatePoint,R=None):
    ...
    ax.semilogx(energies, mean*energies/denergies, drawstyle='steps-post',label=f"{R}:1")
    
    
    return ax


def Run_Plot_Spectrum(n=200,Th_Fa=1,i="All",A_K=0,D_P=0,Base_Sys=str(input("Base System J or G: "))):
    ...
    R = np.round(np.array([50,100,150,200,250,300,350]),6)
    ...
    if i == "All":
        for i in range(len(R)):
            R_val = R[i]
            with openmc.StatePoint('sp.{}.{}.{}ratio.{}.{}.h5'.format(n,ft,R_val,ak,pd)) as sp:
        ...
        with openmc.StatePoint('sp.{}.{}.{}ratio.{}.{}.h5'.format(n,ft,R_val,ak,pd)) as sp:
            SubPlot_Spectrum(ax,sp=sp,R=R_val)
        print("Plot Complete: Flux_Plot.{}.{}.{}ratio.{}.{}.png for R={}ratio".format(n,ft,R_val,ak,pd))
    ...
    if Th_Fa == 0:
        plt.title(r"Graphite System: $E\Phi(E)$ vs E (eV)")
    elif Th_Fa == 1:
        plt.title(r"Graphite System: $E\Phi(E)$ vs E (eV)")    
    plt.legend()
    if i == "All": plt.savefig("Flux_Plot.{}.{}.{}.{}.png".format(n,ft,ak,pd))
    else: plt.savefig("Flux_Plot.{}.{}.{}ratio.{}.{}.png".format(n,ft,R_val,ak,pd))
