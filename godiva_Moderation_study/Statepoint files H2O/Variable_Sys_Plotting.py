...
def System_Data_Plot(SYSTEM="",FTB="Fast",n=200,TEXT_FILE=True,k_range=[0.7,1.6]):
    ...
    if any(_=="Fa" for _ in FTB):
        plt.title(r"α vs $k_{eff}$: Water System Data")
    ...
    if any(_=="Fa" for _ in FTB):
        ...
        plt.scatter(k_Data[0,:],Reactivity[0,:],label="Water",zorder=3)
        plt.plot(k_line,Fa_Coeffs[0]*k_line+Fa_Coeffs[1],label="H2O: {C[0]:.3f}k{C[1]:.3f}".format(C=Fa_Coeffs))
    if any(_=="Th" for _ in FTB):
    ...
    plt.title(r"Water: Alpha-k vs k-Static")
